...
var resizePizzas = function(size) {
    ...
    function changePizzaSizes(size) {
        //used switch cases to change pizza pictures by percentages
        switch (size) {
            case "1":
                newWidth = 25;
                break;
        ...
        //created variable randomPizzas to remove querySelector out of the for-loop
        var randomPizzas = document.querySelectorAll(".randomPizzaContainer");
        //for-loop sets the width of every pizza element to switch case percentage
...
function updatePositions() {
    ...
    // longer loop, and places them in `build` array, which holds these five
    // constant, repeating values:
    for (i = 0; i < 5; i++) {
        build.push(Math.sin((dbs / 1250) + i % 5));
    }


    // Rather than calculating and re-calculating the same
    // five values over and over I stored them in the build array.
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
//Decreased the amount of pizza's being created on the screen from 200 to 48
